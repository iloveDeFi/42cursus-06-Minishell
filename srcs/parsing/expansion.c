...
static char	*ft_get_env_value(char *name, t_env *envList)
{
	...
	while (current != NULL)
	{
		if (ft_strcmp(current->name, name + 1) == 0)
...
void	manage_in_quote(t_token *token, t_env *envList)
{
	char	**splited;
	int		i;
	char	*tmp;
	...
	while (splited[i])
	{
		if (splited[i][0] == '$')
		{
			tmp = ft_get_env_value(splited[i], envList);
			free(splited[i]);
			splited[i] = tmp;
...
void	ft_found_and_replace_usd(t_token **tokens, t_env *envList)
{
	...
	while (tokens[i])
	{
		...
		if (tokens[i]->word[0] == '$')
		{
			...
			{
				tmp = ft_get_env_value(tokens[i]->word, envList);
				free(tokens[i]->word);
				tokens[i]->word = tmp;
